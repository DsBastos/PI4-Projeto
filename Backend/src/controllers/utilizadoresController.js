...
controllers.getAllUtilizador = async (req, res, next) => {
  try {
    const data = await utilizador.findAll({
      include: [
        { model: tipoutilizador, attributes: ['tu_tipo'] },
        { model: regioesTuristicas, attributes: ['rt_nome'] },
      ],
    })
...
controllers.getUtilizadorByEmail = async (req, res, next) => {
  try {
...
controllers.getCountDashboardAT = async (req, res, next) => {
  try {
    ...
    res.send({
      success: true,
      data: [countVisitas, countClientes, countReservas],
    })
  } catch (error) {
    next(error)
  }
